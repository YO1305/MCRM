...
export function KpiExplanation({ role }: { role: KpiPayrollRole }) {
  ...
  return (
    <Card className="space-y-4 bg-slate-50/80">
      ...
      <p className="text-xs leading-relaxed text-muted">
        Лид в блок 2 попадает, если за месяц набраны содержательные шаги менеджера (КП, звонок,
        образцы, этап) — не три пустых «написала». Сделка в 1-м месяце — сразу. С 4-го месяца — нет.
      </p>
    </Card>
  )
}
